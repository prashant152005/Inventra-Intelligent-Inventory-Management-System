...
async function loadCategorySupplierCounts() {
  const token = localStorage.getItem("token");
  if (!token) return;
  ...
  const catRes = await fetch("http://localhost:8080/api/categories/count", { headers });
  document.getElementById("categoryCount").innerText = await catRes.text();

  const supRes = await fetch("http://localhost:8080/api/suppliers/count", { headers });
  document.getElementById("supplierCount").innerText = await supRes.text();
}

loadCategorySupplierCounts();
...
fetch("http://localhost:8080/alerts")
  ...
  .then(alerts => {
      const critical = alerts.filter(a =>
          a.alertType === "LOW_STOCK_NEAR_EXPIRY"
      );
      document.getElementById("combinedCount").innerText = critical.length;
  });
