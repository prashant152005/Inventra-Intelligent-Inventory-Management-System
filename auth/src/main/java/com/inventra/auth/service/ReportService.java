...
public class ReportService {
    ...
    public SalesReportDto generateSalesReport(LocalDate start, LocalDate end) {
        ...
        List<StockTransaction> sales =
                transactionRepo.findByTransactionTypeAndPerformedAtBetween(
        ...
        Map<LocalDate, BigDecimal> dailyRevenue = sales.stream()
                .collect(Collectors.groupingBy(
                        t -> t.getPerformedAt().toLocalDate(),
        ...
        List<DailySalesDto> dailySales = dailyRevenue.entrySet().stream()
                .map(e -> {
                    ...
                    dto.setTransactions((int) sales.stream()
                            .filter(t -> t.getPerformedAt().toLocalDate().equals(e.getKey()))
                            .count());
                    return dto;
                })
    ...
    public String salesReportToCsv(SalesReportDto report) {
        ...
        csv.append("\"Total Revenue\",")
                .append(report.getTotalRevenue().setScale(2, RoundingMode.HALF_UP))
                .append("\n");

        csv.append("\"Total Transactions\",")
        ...
        report.getTopProducts().forEach(p ->
                csv.append("\"").append(p.getProductName()).append("\",")
                        .append("\"").append(p.getSku()).append("\",")
                        .append(p.getQuantitySold()).append(",")
                        .append(p.getRevenue().setScale(2, RoundingMode.HALF_UP))
                        .append("\n")
        );
        ...
        report.getDailySales().forEach(d ->
                csv.append("\"").append(d.getDate()).append("\",")
    ...
    public String inventoryReportToCsv(InventoryReportDto report) {
        ...
        csv.append("Total Stock Value,")
                .append(report.getTotalStockValue()
